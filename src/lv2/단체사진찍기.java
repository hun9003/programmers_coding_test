...
public class 단체사진찍기 {
    static int answer = 0;
    static String[] members = new String[]{"A", "C", "F", "J", "M", "N", "R", "T"};
    static boolean[] visits = new boolean[8];
    public static void main(String[] args) {
        String[] datas = {"N~F=1", "R~T>2"};
        int n = 2;
    ...
    public static void test(String member, String[] datas) {
        if(member.length() == 8) {
            if (check(member, datas)) answer++;
            return;
        }
        for (int i = 0; i < 8; i++) {
            if (!visits[i]) {
                visits[i] = true;
                String tempMember = member + members[i];
                test(tempMember, datas);
                visits[i] = false;
            }
        }
    }

    public static boolean check(String member ,String[] datas) {
        for (String data : datas) {
            int left = member.indexOf(data.charAt(0));
            int right = member.indexOf(data.charAt(2));
            char op = data.charAt(3);
            int index = Integer.parseInt(String.valueOf(data.charAt(4)));

            switch (op) {
                case '<': if(!(Math.abs(left - right) < index+1)) {return false;} break;
                case '>': if(!(Math.abs(left - right) > index+1)) {return false;} break;
                case '=': if(!(Math.abs(left - right) == index+1)) {return false;} break;
            }
        }
