...
class 피로도 {
    ...
    public static int solution(int k, int[][] dungeons) {
        clears = new boolean[dungeons.length];
        playDungeons(dungeons, k, 0, clears);
        return answer;
    }

    public static void playDungeons(int[][] dungeons, int k, int count, boolean[] clears) {
        System.out.println("================================");
        ...
        for(int i = 0; i < dungeons.length; i++) {
            if(k >= dungeons[i][0] && !clears[i]){
                clears[i] = true;
                playDungeons(dungeons, k - dungeons[i][1], count + 1, clears);
                clears[i] = false;
            }
        }
